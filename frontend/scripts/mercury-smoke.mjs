...
try {
 ...
 await page.screenshot({path:`${directory}/desktop-priorities.png`,fullPage:false})
 await page.getByRole('button',{name:'Вся выборка',exact:true}).click()
 await page.screenshot({path:`${directory}/desktop-all-sample.png`,fullPage:false})
 ...
 await page.screenshot({path:`${directory}/mobile-priorities.png`,fullPage:true})
 await page.getByRole('button',{name:'Вся выборка',exact:true}).click()
 await page.screenshot({path:`${directory}/mobile-all-sample.png`,fullPage:true})
 assert(await page.getByRole('button',{name:'Вся выборка',exact:true}).isVisible())
 await page.getByRole('button',{name:'Переводы',exact:true}).click()
 await page.screenshot({path:`${directory}/mobile-transfers.png`,fullPage:true})
 assert.deepEqual(errors,[])
 console.log(JSON.stringify({status:'passed',checks:['adjacent-edge','real-transfers','validated-routes','resilience-prefix-5','real-anomalies','roomy-participant-queue','mobile-identity','mobile-no-overflow','no-console-errors'],screenshots:directory}))
