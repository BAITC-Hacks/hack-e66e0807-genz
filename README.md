...
Она при необходимости создаёт `.venv` и устанавливает зависимости, собирает интерфейс, пересчитывает данные и запускает сервер. Для подготовленной машины доступно `python3 run.py --skip-install`; другой порт — `--port 8001`. Проверка без постоянного сервера — `python3 run.py --check` (сборка, расчёт и независимые валидаторы). Финальный запуск `python3 run.py --data <каталог исходных файлов> --check` проверен из чистого архива исходников без `.venv` и `node_modules`: установка, сборка и все независимые проверки прошли; повторные полные расчёты заняли 4,325 и 4,375 секунды. Установка использовала локальный кэш пакетов. Это проверка нового окружения на текущей машине, не всех операционных систем; при первом скачивании время зависит от сети.
...
1. В разделе «Участники» откройте «Приоритеты»: просторная таблица содержит топ-50, полный gid, роль и балл. Выберите строку — справа появится основание приоритета: оборот, число связей, пути от исходных клиентов (`seed`) и посредничество. Кнопка «Исследовать связи» открывает граф выбранного участника.
...
| Провайдер, запросы, промпт, инструменты и проверка AI-ответов | [Устройство AI-ассистента](docs/AI-ASSISTANT.md) |

Программа использует графовые алгоритмы и детерминированные правила. Обученная ML-модель для расчёта ролей не используется; `role_score` не является вероятностью. Необязательный LLM-ассистент отделён от расчёта и требует отдельной настройки (см. ниже).
...
Это признаки совпадения дат и нагрузки, а не доказательство происхождения средств. Объяснения и предложения запросить данные формируются проверяемыми правилами и доступны без AI.
...
## Необязательный AI-ассистент

Базовый продукт, расчёт и три CSV работают локально **без ключей и платных сервисов**. Кейс разрешает интернет для внешнего LLM API. Реализован отдельный адаптер к бесплатному маршрутизатору OpenRouter (`openrouter/free`); платная модель автоматически не подставляется. Доступность и лимиты бесплатного сервиса зависят от провайдера.

Для включения создайте собственный ключ на https://openrouter.ai/keys, скопируйте `.env.example` в `.env` в корне проекта и заполните `OPENROUTER_API_KEY`. Перезапустите `python3 run.py`. `.env` исключён из Git, ключ не публикуется и не передаётся браузеру. Проверяющий использует собственный ключ; он не нужен для воспроизведения обязательного результата. Без настройки пункт ассистента скрыт.

Браузер отправляет вопрос на локальный `POST /api/assistant/query`. Сервер обращается к `https://openrouter.ai/api/v1/chat/completions`: передаёт инструкцию отвечать только по вычисленным свидетельствам, вопрос и схемы разрешённых инструментов. Модель выбирает инструмент чтения отчёта (узел, связи, общие получатели, приоритеты, сообщество, маршруты, устойчивость или аномалии); сервер выполняет его над локальным JSON и возвращает ограниченные результаты. Исходные parquet целиком не отправляются, но вопрос и выбранные данные отчёта передаются внешнему провайдеру. Запросы к произвольным файлам, SQL и выполнение кода отсутствуют.

Ответ модели проверяется по ссылкам и значениям отчёта. Для общего получателя количество плательщиков и суммы заново вычисляются сервером по полному набору соответствующих рёбер. Лимиты: 4 раунда, 20 строк на инструмент, 16 КБ свидетельств, 20 секунд. При недостаточных данных или невалидном ответе сервер возвращает явную ошибку, а не выдуманный результат.

**Проверено с реальным провайдером:** общий получатель пяти источников (5 плательщиков, 179 500 KZT, пять проверенных ссылок на рёбра), причина приоритета выбранного узла и вопрос «Как отбираются участники для приоритета». Последний сценарий дополнительно пройден через браузер. Методика читается инструментом `get_priority_method` из правил аналитического модуля; пример балла ссылается на реальный узел. Произвольные вопросы вне доступных свидетельств могут получить отказ. Бесплатный API имеет внешние лимиты и не гарантирует доступность; это не блокирует локальное решение.

Технический разбор промпта, инструментов, проверок и ошибок — [AI-ассистент](docs/AI-ASSISTANT.md). После настройки ключа воспроизводимая живая проверка: `.venv/bin/python scripts/verify_assistant_live.py`.

## Подтверждение готовности

[Результаты проверки по критериям кейса](docs/VALIDATION.md) содержат измерения, проверенные сценарии интерфейса, дополнительные аналитические функции и ограничения. Основной путь: найти gid → выбрать узел → прочитать основание приоритета → выбрать стрелку и точную сумму → увидеть ограничения → скачать три CSV. Интерфейс проверен на широком экране и ширине 390 px.
