...
Сам аналитический расчёт на тестовой машине занимает около **0,20 секунды**; независимая проверка измеряет также полный процесс CLI со стартом Python и требует менее 300 секунд для каждого из двух прогонов. Точные времена текущей машины печатает `scripts/verify_delivery.py`.
