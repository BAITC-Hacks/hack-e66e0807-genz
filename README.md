...
Она при необходимости создаёт `.venv` и устанавливает зависимости, собирает интерфейс, пересчитывает данные и запускает сервер. Для подготовленной машины доступно `python3 run.py --skip-install`; другой порт — `--port 8001`. Проверка без постоянного сервера — `python3 run.py --check` (сборка, расчёт и независимые валидаторы). На подготовленной машине `python3 run.py --skip-install --check` прошла; это не утверждение о проверке установки на любой чистой машине.
...
Python, pandas и pyarrow читают и проверяют parquet; NetworkX рассчитывает графовые метрики и сообщества. React, TypeScript, Vite, Tailwind CSS и компоненты shadcn/ui дают интерфейс. Python-сервер публикует сборку и разрешённые файлы результата только по `127.0.0.1`. Между расчётом и UI — [зафиксированный контракт](docs/DATA-CONTRACT.md), без базы данных и облачной службы. [Архитектура подробнее](docs/ARCHITECTURE.md).
