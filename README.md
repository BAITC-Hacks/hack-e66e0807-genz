...
**Статус проверки:** интеграционные проверки с контролируемыми ответами пройдены; реальный обмен с OpenRouter пока не подтверждён, поэтому AI не включён в заявленные завершённые возможности. После настройки ключа отдельная живая проверка: `.venv/bin/python scripts/verify_assistant_live.py`.
