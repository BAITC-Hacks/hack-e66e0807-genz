...
Результат — интерпретируемая модель наблюдаемой структуры переводов. Она сочетает графовые метрики, Louvain и правила; обучение по меткам и нейросеть для расчёта ролей не используются. Необязательный внешний LLM читает готовые свидетельства и не меняет оценки. Задача — ранжировать направления проверки, а не автоматически определить преступление. Формулы ниже соответствуют `solution/analytics.py` и `solution/temporal.py`.
...
Это признаки совпадения дат и нагрузки, а не доказательство происхождения средств. Объяснения и предложения запросить данные формируются проверяемыми правилами без LLM. Необязательный адаптер AI описан в README; реальные сценарии и ограничения проверки описаны в docs/AI-ASSISTANT.md.
