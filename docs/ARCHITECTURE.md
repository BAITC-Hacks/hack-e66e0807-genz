...
«Граф денег» — локальное пакетное приложение. Входные parquet находятся в корневом каталоге `data/` и входят в репозиторий. Аналитический процесс один раз строит проверенный снимок сети; браузер исследует этот снимок. Запрос выбора счёта не запускает пересчёт. База данных, облачный сервис и внешний API не требуются.
...
| `solution/server.py` | Каталоги результата и сборки → HTTP | Раздача разрешённых файлов на loopback и необязательный API ассистента |
...
После базовых ролей и временных признаков `pipeline` вызывает `anomalies.enrich_anomalies`, `routes.find_routes` и `resilience.calculate_resilience`. Они добавляют объяснимые признаки в JSON, сохраняя обязательные CSV. Маршруты ограничивают перебор кандидатов и дат; сценарии удаления используют представления оставшегося графа; сравнение аномалий исключает несопоставимые цензурированные метрики. `scripts/verify_extensions.py` независимо сверяет опубликованные результаты с исходными таблицами. При росте до миллиона узлов сравнение по группам следует заменить предварительно рассчитанными статистиками, полный JSON — выдачей окрестностей по запросу, а ограниченный обход выполнять лениво.


### Необязательный внешний ассистент

`solution/assistant.py` читает готовый отчёт и исполняет ограниченные инструменты модели. `solution/server.py` предоставляет `GET /api/assistant/status` и `POST /api/assistant/query`, проверяет Host, Origin, тип и размер тела. Ключ OpenRouter читается из окружения или игнорируемого `.env`; браузер ключ не получает. Без ключа адаптер выключен, статические данные и CSV доступны. Бесплатная модель `openrouter/free` вызывается через HTTPS, разрешённые факты и цитаты проверяются сервером. Это отдельная внешняя интеграция, не зависимость аналитического пайплайна. Реальный вызов требует отдельной проверки с ключом.

Подробности запросов, промпта, инструментов, статусов и проверки цитат: [AI-ассистент](AI-ASSISTANT.md).
